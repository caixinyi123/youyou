...
window.onload = function initCube() {
    ...
    suoLoc = gl.getUniformLocation(program, "suo");

    document.getElementById("xTans").onclick = function () {
        axis = xAxis;
    }

    document.getElementById("yTans").onclick = function () {
        axis = yAxis;
    }

    document.getElementById("zTans").onclick = function () {
        axis = zAxis;
    }
	
    document.getElementById("SF").onchange = function(event){
        suo=event.target.value;
    }

    document.getElementById("xAdvect").onchange = function(event){
        pos[0] = event.target.value;  
    }

    document.getElementById("yAdvect").onchange = function(event){
        pos[1] = event.target.value;  
    }

    document.getElementById("zAdvect").onchange = function(event){
        pos[2] = event.target.value;  
    }
...
function render() {
    ...
    theta[axis] += 2*Math.PI/180;
    gl.uniform3fv(thetaLoc, theta);
    gl.uniform3fv(posLoc,pos);
    gl.uniform1f(suoLoc,suo);
